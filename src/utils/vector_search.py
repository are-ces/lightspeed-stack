...
async def _fetch_byok_rag(
    ...
    try:
        ...
        # Get vector store IDs to query (either specified or all available)
        all_vector_store_ids = await get_vector_store_ids(client, vector_store_ids)

        # Filter out Solr vector stores
        vector_store_ids_to_query = [
            vs_id
            for vs_id in all_vector_store_ids
        ...
        results_per_store = await asyncio.gather(*tasks)
        ...
        # Resolve vector_store_id to user-facing rag_id for all results
